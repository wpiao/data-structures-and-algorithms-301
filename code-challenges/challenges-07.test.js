...
const splitFoods = recipe => recipe.ingredients.map(element => element.split(' ').slice(2).join(' '));
...
const removeVowels = str => {
  // Solution code here...
  const vowels = {
    a: 'a is one of the vowels',
    e: 'e is one of the vowels',
    i: 'i is one of the vowels',
    o: 'o is one of the vowels',
    u: 'u is one of the vowels',
  };

  return str.split('').filter(letter => vowels[letter] === undefined).join('');
};
...
const extractVowels = str => {
  ...
  return [remainedStr, removedStr.split('').sort().join('')];
};
