...
const longestString = arr => {
  // Solution code here...
  if (arr.length) {
    const longest = arr.reduce((prev, current) => current.length > prev.length ? current : prev, arr[0]);
...
const findHappiness = arr => arr.filter(string => (/:\)/).test(string));
...
const standardizePhoneNumbers = arr => arr.map(number => number.slice(1, 4) + number.slice(6, 9) + number.slice(10));
