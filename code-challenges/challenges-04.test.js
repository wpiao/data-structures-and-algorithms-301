...
const updateAnimal = (arr, callback) => {
  // Solution code here...
  const result = [];
  arr.forEach(element => result.push(callback(element)));
  return result;
};
...
const sortMeetingsByDay = arr => arr.sort((a, b) => {
  ...
  if (sorter[a.dayOfWeek.toLowerCase()] !== sorter[b.dayOfWeek.toLowerCase()]) {
    return sorter[a.dayOfWeek.toLowerCase()] - sorter[b.dayOfWeek.toLowerCase()];
  } else {
    return 0;
  }
  // if (sorter[a.dayOfWeek.toLowerCase()] < sorter[b.dayOfWeek.toLowerCase()]) {
...
const sortSchedule = arr => arr.sort((a, b) => {
  ...
  const convertToHour = time => {
    return parseInt(time.slice(0, 2)) + (parseInt(time.slice(2)) / 60);
  };
  ...
  } else if ((convertToHour(a.end) - convertToHour(a.start)) !== (convertToHour(b.end) - convertToHour(b.start))) {
    return (convertToHour(a.end) - convertToHour(a.start)) - (convertToHour(b.end) - convertToHour(b.start));
  } else {
    return 0
  }
  // if (sorter[a.dayOfWeek.toLowerCase()] < sorter[b.dayOfWeek.toLowerCase()]) {
