...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = arr => arr.reduce((accumulator, currentValue) => currentValue > accumulator ? currentValue : accumulator);
...
const hasChildrenEntries = (arr, character) => {
  for (const person of arr) {
    if (person.name === character) {
      return Object.entries(person).filter(property => property[0] === 'children').length ? true : false;
    }
  }
  return false;
};
...
const totalCharacters = arr => arr.reduce((accumulator, currentValue) => {
...
const houseSize = arr => {
  ...
  arr.forEach(person => {
    ...
    if (person.children) {
      members += person.children.length;
    }
...
const houseSurvivors = arr => {
  ...
  arr.forEach(element => {
    ...
    if (element.children) {
      survived += element.children.length;
    }
    obj.members = survived;
    survivors.push(obj);
  });
  return survivors;
};
