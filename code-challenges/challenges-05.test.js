...
const countNumberOfElements = arr => arr.reduce(answerSoFar => answerSoFar + 1, 0);
...
const returnNames = arr => arr.reduce((answerSoFar, currentValue) => {
...
const reversedString = str => {
  ...
  const reversedArray = strArr.reduce((answerSoFar, currentValue) => currentValue + answerSoFar, '');
...
const countNumberOfChildren = arr => arr.reduce((answerSoFar, currentValue) => {
  if (currentValue.children) {
    answerSoFar += currentValue.children.length;
  }
  return answerSoFar;
}, 0);
...
const calculateAverage = arr => {
  const finalAccumulator = arr.reduce((answerSoFar, currentValue) => {
    answerSoFar.sum += currentValue;
    answerSoFar.count++;
    return answerSoFar;
  }, { sum: 0, count: 0 });
  return finalAccumulator.sum / finalAccumulator.count;
}
...
const countPrimeNumbers = arr => arr.reduce((answerSoFar, currentValue) => isPrime(currentValue) ? answerSoFar + 1 : answerSoFar, 0);
...
const extractStat = (statName, arr) => arr.reduce((answerSofar, currentValue) => !answerSofar ? (currentValue.stat.name === statName ? currentValue : answerSofar) : answerSofar, null);
...
const extractChildren = arr => {
  // Solution code here...
  const filteredArr = arr.filter(character => character.name.includes('a'));
  const result = filteredArr.reduce((answerSoFar, currentValue) => {
    if (currentValue.children) {
      for (let child of currentValue.children) {
        answerSoFar.push(child);
      }
    }
    return answerSoFar;
  }, []);
