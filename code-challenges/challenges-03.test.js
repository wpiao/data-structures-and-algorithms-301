...
const containsAnd = arr => arr.filter(word => word.includes('and'));
...
const oddValues = arr => arr.filter(integer => integer % 2 === 1);
...
const getStatName = (arr, minBaseStat) => arr.filter(stat => stat.baseStat > minBaseStat).map(statObj => statObj.stat.name);
...
const evenOddNumericValues = arr => arr.filter(element => typeof (element) === 'number')
  .map(num => {
    if (num % 2 === 0) {
      return 'even';
    } else {
